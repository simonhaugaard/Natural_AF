...
#### MA 
```{r ma_plots, fig.height=7, fig.width=7}
# Function to generate MA plots
plot_MA <- function(df, contrast_name) {
  ggplot(df, aes(x = AveExpr, y = logFC, color = adj.P.Val < 0.05)) +
...
# Generate and save MA plots
output_dir <- "../output/MA_plots/"


for (contrast in names(res)) {
  p <- plot_MA(res[[contrast]], contrast)
  
  # Save the plot as PNG
  ggsave(filename = paste0(output_dir, "MA_", contrast, ".png"), 
         plot = p, width = 7, height = 7, dpi = 300)
  
  print(p) # Display in R Markdown
}

# Combine all results into a single dataframe for faceting
res_combined <- bind_rows(res, .id = "Contrast")

# Generate a combined MA plot using facet_wrap
ggplot(res_combined, aes(x = AveExpr, y = logFC, color = adj.P.Val < 0.05)) +
  geom_point(alpha = 0.5, size = 0.7) +
  scale_color_manual(values = c("black", "red"), labels = c("Not Significant", "Significant")) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "blue") +
  labs(title = "MA Plots for All Contrasts",
       x = "Average Expression (log2)",
       y = "Log Fold Change",
       color = "Significance") +
  facet_wrap(~ Contrast, nrow = 3, scales = "free") +
...
# Save the combined MA plot
ggsave("../output/MA_plots/MA_combined.png", width = 12, height = 12, dpi = 300)

## Density plot 
# Define contrasts to keep
selected_contrasts <- c("AF_vs_control_RA", "AF_vs_control_LA", 
                         "RA_vs_LA_control", "AF_vs_control_RV", 
                         "AF_vs_control_LV", "RV_vs_LV_control", "AverageRegionEffect_AV_healthy")

# Combine all contrast results and filter only selected contrasts
res_filtered <- bind_rows(res, .id = "Contrast") %>%
  filter(Contrast %in% selected_contrasts)

# Plot density distributions of logFC (Filtered contrasts)
ggplot(res_filtered, aes(x = logFC, color = Contrast)) +
  geom_density(size = 1, alpha = 0.7) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  labs(title = "Density Distribution of logFC (Selected Contrasts)",
...
# Save the plot
ggsave("../output/logFC_density_selected.png", width = 12, height = 10, dpi = 300)
```
