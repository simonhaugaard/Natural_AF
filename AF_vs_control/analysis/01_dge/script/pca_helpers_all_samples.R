...
create_publication_pca_plot_with_loadings <- function(pca_df, loadings_df, x, y, color_var, shape_var = NULL, colors, shapes = NULL, title_text = "", show_labels = TRUE, scaling_factor = 150, add_ellipse = TRUE) {
  # Create PCA plot with sample points
  pca_plot <- ggplot(pca_df, aes_string(x = x, y = y, color = color_var)) +
    geom_point(aes_string(shape = shape_var), size = 5, alpha = 0.9, stroke = 1.2) +  
  ...
  # Apply shape aesthetics if shape_var is provided
  if (!is.null(shape_var) && !is.null(shapes)) {
    pca_plot <- pca_plot + scale_shape_manual(values = shapes)
  }
...
generate_and_save_pca_plots <- function(pca_df, loadings_df, dimensions, colors, shapes = NULL, base_path = "../output/", add_ellipse = TRUE, shape_var = "Group") {
  for (dim in dimensions) {
    ...
    pca_plot_with_labels <- create_publication_pca_plot_with_loadings(
      ...
      y = y_dim,
      color_var = "Region",
      shape_var = shape_var,  # Include shape variable
      colors = colors,
      shapes = shapes,  # Include shape aesthetic
      title_text = paste("PCA Plot of Horse Transcriptome with Loadings:", x_dim, "vs", y_dim),
      show_labels = TRUE,
    ...
    pca_plot_without_labels <- create_publication_pca_plot_with_loadings(
      ...
      colors = colors,
      shapes = shapes,  # Include shape aesthetic
      title_text = paste("PCA Plot of Horse Transcriptome with Loadings (No Labels):", x_dim, "vs", y_dim),
      show_labels = FALSE,
      add_ellipse = add_ellipse  # Add ellipses based on function argument
    )
    
    # Save each plot separately
    ggsave(filename = paste0(base_path, "PCA_Horse_Loadings_", x_dim, "_", y_dim, "_With_Labels.png"),
           plot = pca_plot_with_labels, width = 8, height = 6, dpi = 600)
    ggsave(filename = paste0(base_path, "PCA_Horse_Loadings_", x_dim, "_", y_dim, "_No_Labels.png"),
